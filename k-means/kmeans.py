...
def single_k_means(k, pixel_list, pixel_weights):
    ...
    while True:
        ...
        for mean in means_average_dictionary:
            means_set.add(tuple([int(means_average_dictionary[mean][0][i] / means_average_dictionary[mean][1]) for i in range(3)]))
        for pixel in pixel_weights:
            min_distance = float('inf')
            best_mean = ()
            for mean in means_set:
        ...
        if pixel_mapping == old_pixel_mapping:
            return means_set, pixel_mapping
        old_pixel_mapping = pixel_mapping.copy()
        pixel_mapping = {}
        iterations += 1
...
means_set, pixel_mapping = run_k_means(k, pixel_dictionary)
for x in range(img.size[0]):
    for y in range(img.size[1]):
...
print('Final means:')
for idx, tup in enumerate(sorted([(sum_by_means[mean], mean) for mean in means_set], reverse=True)):
    print('{}: {} => {}'.format(idx + 1, tup[1], tup[0]))
img.save(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'output.jpg'))
print('Time elapsed: {}s'.format(adj_sig_figs(3, time() - start_time)))
